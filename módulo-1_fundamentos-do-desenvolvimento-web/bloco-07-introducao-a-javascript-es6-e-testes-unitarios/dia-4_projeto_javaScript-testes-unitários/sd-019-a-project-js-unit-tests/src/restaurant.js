...
const createMenu = (foodAndDrink) => {
  const objectRestaurant = {
    fetchMenu: () => foodAndDrink,
    consumption: [],
    order: (string) => { objectRestaurant.consumption.push(string); },
    pay: () => {
      let payment = 0;
      if (Object.keys(objectRestaurant.fetchMenu())[0] === 'food' 
      || Object.keys(objectRestaurant.fetchMenu())[1] === 'drink') {
        const foodObj = objectRestaurant.fetchMenu().food;
        const drinkObj = objectRestaurant.fetchMenu().drink;
        const theAssign = Object.assign(foodObj, drinkObj);
        const theMenu = Object.values(theAssign);
        for (let i = 0; i < theMenu.length; i += 1) {
          payment += +theMenu[i];
        }

      return +(payment * 1.1).toFixed(2);
      }
    },
  };
...
/* const theRestaurant = createMenu(
  { food: { coxinha: 3.9, coxinha2: 3.9 },
  drink: { agua: 3.9 },
  },
);

console.log(theRestaurant.fetchMenu());
theRestaurant.order(`${Object.keys(theRestaurant.fetchMenu().food)},${Object.keys(theRestaurant.fetchMenu().drink)}`);
console.log(theRestaurant.consumption);
console.log(theRestaurant.pay()); */
