...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    // ```
    expect(createMenu({ food: { coxinha: 3.9, sopa: 9.9 }, drink: { agua: 3.9, cerveja: 6.9 } }).fetchMenu())
    .toEqual({ food: { coxinha: 3.9, sopa: 9.9 }, drink: { agua: 3.9, cerveja: 6.9 } })
    ...
    // ```
    expect(createMenu({ objetoQualquer: { copos: 4, pratos: 5 } }).fetchMenu()).toEqual({ objetoQualquer: { copos: 4, pratos: 5 } });
    ...
    for (let i = 0; i <= objetoRetornado.consumption.length; i += 1) {
      objetoRetornado.consumption.shift();
    }
    ...
    // ```
    expect(createMenu({ food: { coxinha: 3.9, coxinha2: 3.9 },
      drink: { agua: 3.9 },
      }).pay()).toEqual(12.87);
